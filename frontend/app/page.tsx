...
export default function Home() {
  ...
  const columns: ColumnDef<Stock>[] = [
    ...
    },
    {
      ...
      header: "Investment",
      cell: (info) => `₹${(info.getValue() as number).toLocaleString('en-IN', {maximumFractionDigits: 2})}`
    },
    {
      ...
      cell: (info) => {
        ...
        return (
          <span className={value >= 0 ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>
            ₹{value.toLocaleString('en-IN', {maximumFractionDigits: 2})} ({percent.toFixed(2)}%)
          </span>
        )
      }
    },
    {
      ...
      cell: (info) => {
        ...
        return value && value !== '#N/A' ? (typeof value === 'number' ? value.toFixed(2) : value) : 'N/A';
      }
    },
    {
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className={`text-2xl font-bold ${totalGainLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ₹{totalGainLoss.toLocaleString('en-IN', {maximumFractionDigits: 2})}
            </p>
          </div>
        </div>
